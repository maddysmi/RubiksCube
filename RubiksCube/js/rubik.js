...
//	OZ.CSS3.set(this._node, "backface-visibility", "hidden");
	...
	switch (type) {
		case Face.LEFT:
			...
			//OZ.CSS3.set(this._node, "background", "url(img/10.jpg)no-repeat");
			//OZ.CSS3.set(this._node, "background", "url(https://source.unsplash.com/random/?tech,studied)no-repeat");
			//OZ.CSS3.set(this._node, "background-size", "cover");
		break;
		case Face.RIGHT:
...
Face.prototype.setImagePath = function(path) {
	this._imagePath = path;
	this._node.style.backgroundImage = "url('"+path+"')";
	this._node.style.backgroundSize = "100%"

}

Face.prototype.getImagePath = function() {
...
Cube.prototype.setFace = function(type, color, path) {
	...
	this._faces[type].setImagePath(path);
}
...
Cube.prototype.prepareColorChange = function(sourceCube, rotation) {
	...
	for (var p in sourceFaces) {
		...
		this._tmpFaces[targetType] = sourceFaces[sourceType].getImagePath();
	}
}
...
//	parent.removeChild(this._node);
	...
	for (var p in this._tmpFaces) {
		var type = parseInt(p);
		this.setFace(type, this._tmpFaces[p], this._tmpFaces[p]);
	}
	this._tmpFaces = {};
...
Rubik.prototype.randomize = function() {
	...
	var cb = function() {
		...
		if (remain > 0) {
			...
			this._help.a = OZ.DOM.elm("p", {innerHTML:"Spiele mit dem Cube in dem du auf dem Hintergrund oder dem Cube ziehst."});
			this._help.b = OZ.DOM.elm("p", {innerHTML:"Viel Spass!"});
	...
	for (var z=0;z<Rubik.SIZE;z++) {
		for (var y=0;y<Rubik.SIZE;y++) {
			for (var x=0;x<Rubik.SIZE;x++) {
				...
				if (z == 0) { cube.setFace(Face.FRONT, "black", "img/10.jpg"); }
				if (z == 2) { cube.setFace(Face.BACK, "black", "img/11.jpg"); }

				if (x == 0) { cube.setFace(Face.LEFT, "black", "img/13.jpg"); }
				if (x == 2) { cube.setFace(Face.RIGHT, "black", "img/29.jpg"); }

				if (y == 0) { cube.setFace(Face.TOP, "black","img/48.jpg"); }
				if (y == 2) { cube.setFace(Face.BOTTOM, "black", "img/49.jpg"); }
